function [ negative_log_likelihood] = get_likelihood_and_q(as, rs, ss, alpha, beta, gamma, initial_Q)
...
for i = 1:size(as,1)-1
    ...
    if isnan(log(normalized_p_as(as(i,1))))
        number_of_nans = sum(isnan(normalized_p_as));
        normalized_p_as(isnan(normalized_p_as)) = 1/number_of_nans;
    end
